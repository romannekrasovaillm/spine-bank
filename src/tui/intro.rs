...
//! Пропуск — любая клавиша; реиграция из чата — `/intro`.
...
fn dialog_lines(ticks: u32, theme: &Theme) -> Vec<Line<'static>> {
...
fn draw_session(f: &mut Frame, area: Rect, theme: &Theme, ticks: u32) {
    ...
    let lines = dialog_lines(ticks, theme);
