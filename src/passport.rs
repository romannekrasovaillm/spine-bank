...
fn collect_claims(repo: &Path, report: &GateReport) -> Vec<Claim> {
    ...
    // отчётах (ADR-048). Уровень `none` здесь не называется: там независимости
    // нет вовсе, и это видно гейту находкой `judge_is_author`.
    out.extend(judge_independence_claims(repo)); // Смысловой слой (ADR-052): включённая составляющая — это суждение
    // модели, и паспорт обязан сказать, чьё именно и независимо ли оно;
    // выключенная — назвать, какая рубрика закрывает класс дефекта, иначе
    // «механикой не проверяется» читается как приговор без выхода.
    if let Some(semantic) = report
        .components
        .iter()
        .find(|c| c.name == "semantic_quality")
    {
        if semantic.status == GateStatus::Skip {
            out.push(Claim {
                source: "смысл".to_string(),
                text: format!(
                    "смысл решения, ссылок и кода механикой не проверяется ({})",
                    semantic.detail
                ),
                instead: "включите `semantic_quality` в [gate.required] и прогоните \
                          смысловую рубрику по досье: `adr_spine_consistency` — \
                          решение против инварианта, `model_link_semantics` — \
                          ссылка не на ту сущность, `nfr_mechanism_fit` — \
                          обещание без механизма, `code_invariant_conformance` — \
            ...
            out.push(Claim {
                source: "смысл".to_string(),
                text: format!(
                    "проверено СУЖДЕНИЕМ модели ({}), а не доказательством: механика \
                     сверила хэш досье, порог и подтверждённость цитат, но не \
                     качество суждения и не верность вывода",
                    semantic.detail
                ),
                instead: "для класса D6/D10/D11 это лучшее, что есть: прочитайте \
                          находки составляющей и отчёты в reports/rubric/ — они \
                          называют цитаты обеих сторон"
                    .to_string(),
            });
        }
    }
