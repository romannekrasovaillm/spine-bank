...
/// Резолвится ли `baseline` в коммит репозитория — тот же критерий, что у
/// [`rehearse`] (`git cat-file -t` → `commit`).
///
/// `None` — каталог не git-репозиторий: проверка смысла не имеет, и вызывающий
/// обязан промолчать, а не объявить якорь нерезолвящимся. `Some(false)` —
/// репозиторий есть, а коммита по этому имени нет: отчёт, объявленный
/// пройденным на таком якоре, — ложная аттестация (настоящая репетиция его
/// произвести не может: [`rehearse`] падает на нерезолвящемся якоре).
#[must_use]
pub fn baseline_resolves(repo: &Path, baseline: &str) -> Option<bool> {
    // Первый вызов отвечает на «это репозиторий?», второй — на «есть ли такой
    // коммит?». Ненулевой выход `cat-file` — это и есть искомое «нет», а не
    // отсутствие репозитория: `?` здесь превратил бы находку в молчание.
    git_out(repo, &["rev-parse", "--git-dir"])?;
    Some(git_out(repo, &["cat-file", "-t", baseline]).is_some_and(|k| k.trim() == "commit"))
}
