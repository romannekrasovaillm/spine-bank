...
fn judge_extra_rules(rubric: &Rubric) -> String {
    ...
    if !roles.is_empty() {
        ...
        // E9.1/E9.2: структурированные цитаты с указателем на источник —
        // основной канал; проверяются механически по названному источнику.
        out.push_str(
            "- ОСНОВНОЙ канал доказательств — цитаты с указателем на источник: в каждый элемент \
             scores добавь поле \"citations\": [{\"role\": \"<роль>\", \"source\": \
             \"<путь источника>\", \"quote\": \"<дословный фрагмент>\"}]. source — путь ровно \
             как в маркере источника (маркер «=== ИСТОЧНИК subject: src/pay.py ===» → \
             \"src/pay.py\"; «=== ИСТОЧНИК reference: ARCHITECTURE-SPINE.md#AD-1 ===» → \
             \"ARCHITECTURE-SPINE.md#AD-1\"), role — роль из того же маркера. Механика сверяет \
             фрагмент с НАЗВАННЫМ источником и роль — с составом досье: цитата из кода не \
             засчитывается за цитату из инварианта, а указатель на несуществующий источник \
             обнуляет цитату. Цитаты в rationale оставь — это страховка, если структура не \
             сложилась;\n",
        );
    }
    if needs_coverage {
