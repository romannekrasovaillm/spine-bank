...
pub struct JudgeConfigSnapshot {
    ...
    /// E8.1: собирались ли сэмплы адаптивно. Аддитивное поле: у отчётов до
    /// 0.3.9 его нет, и отсутствие читается как `false` — «сэмплов ровно
    /// `samples`», то есть прежнее поведение.
    #[serde(default)]
