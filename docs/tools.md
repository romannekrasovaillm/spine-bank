...
| `model_query` | Запрос к типизированной модели архитектуры (`model/`, ADR-003): карточка сущности по ID со связями и обратными ссылками, либо список сущностей с фильтром по типу. Толерантная загрузка (E3): работа по валидному подмножеству, в ответе `load_issues` (битые файлы с причинами) | `dir` (каталог модели, по умолчанию `model`); `id` (карточка); `type` (фильтр: `cmp`, `adr`, …) |
| `trace_check` | Трассируемость как fitness-функция (ADR-006): покрытие звеньев REQ → NFR → AD/ADR → CMP → правило CONSTRAINTS.yaml, поимённые сироты, сверка модели со spine; AD без правила и без `unverifiable` — error. Отчёт markdown (годится для evidence bundle); толерантная загрузка модели (E3) — `load_issues` | `dir` (корень кейса, по умолчанию текущий каталог) |
| `adr_new` | ADR по шаблону AI-DLC в `docs/adr/` (очередной номер, kebab-title); `author_model` пишется в шапку (`- Модель-автор: …`) — судья берёт автора из документа, а не со слов в момент оценки (ADR-048) | `title`*; `author_model` |
| `spine_lint` | Линтер ARCHITECTURE-SPINE.md: дубли AD-id, пустые Binds/Prevents/Rule, заглушки, непиннутые версии | `path`* |
| `fitness_check` | Fitness functions из CONSTRAINTS.yaml по репозиторию → PASS/FAIL с находками `file:line`; путь реестра — единый резолвер (явный `constraints` → `.arch-handoff/CONSTRAINTS.yaml` → корневой `CONSTRAINTS.yaml`); MCP-вердикт несёт `constraints` (использованный путь) и `drift_note` (дрейф двух копий); правила с неизвестными типами — warn `unknown_rule_type` (аддитивное поле `skipped_unknown` — в CLI `--json` и у `rules_report`); модель доверия (ADR-053): в MCP-режиме `command_succeeds` по умолчанию не исполняются (no-exec) — пропуски `command_untrusted` в поле `untrusted_skipped`, снятие — `ARCH_NO_EXEC=0` в окружении сервера; CLI — флаг `--no-exec`, allow-файл `rules allow` | `repo`*; `constraints` (путь к YAML, иначе авто-резолв) |
| `model_validate` | Ссылочная целостность модели (`model/`, ADR-003): битая ссылка / дубль ID / цикл `depends_on` — error; ADR без CMP, NFR без проверки, QAS без сценария — warn. Толерантная загрузка (E3): сущность с битым frontmatter — error-находка `load-error`, валидные проверяются. Ответ — JSON `{passed, issues, summary}` (мост в MCP, read-only) | `dir`/`path` (корень кейса ИЛИ каталог `model/` — инструмент находит модель сам, T-13; по умолчанию `model`) |
| `model_drift` | Дрейф «модель ↔ код» (корень кейса ИЛИ каталог `model/` — инструмент находит модель сам, T-13): CMP с несуществующим путём `code_roots` (ADR-030) — error; каталог с манифестом сборки (тот же набор, что у `reverse_survey`) без покрывающего `code_roots` — warn; звено `INT → контракт` в семантике `trace_check` (ADR-035: битый путь `contract` — error, поле не задано — warn). Находки с `adr`/`rationale`/`fix_hint`. Толерантная загрузка (E3) — `load_issues`. Ответ — JSON `{passed, issues, summary}` (мост в MCP, read-only). CLI-эквивалент: `arch-be model drift <dir> [--json]` | `dir` (корень кейса, по умолчанию текущий каталог) |
| `nfr_check` | Количественные NFR поверх модели (ADR-007): `budget` (сумма бюджетов hop'ов INT против p99), `availability` (композиция против SLA + RTO/RPO), `capacity` (RPS против ёмкости), `cost` (TCO + цена выхода); `all` — все четыре. Толерантная загрузка (E3) — `load_issues`. Ответ — JSON `{passed, issues (с виновными hop'ами/звеньями), summary}` (мост в MCP, read-only) | `path`* (корень кейса — каталог с `model/`); `kind` (вид проверки, по умолчанию `all`) |
| `delta_guard` | Гейт прямых правок спайна мимо дельты (модель 5.2): изменённые защищённые файлы (дефолт `model/`, `ARCHITECTURE-SPINE.md`, `CONSTRAINTS.yaml`) обязаны упоминаться в дельте-покрытии — активной или заархивированной внутри проверяемого диапазона. Ответ — JSON `{passed, violations, covered, mentions, active_deltas, archived_in_range, summary}` (мост в MCP, read-only) | `path` (репозиторий, по умолчанию `.`); `base` (база diff, по умолчанию HEAD); `protect` (список, заменяет дефолт) |
...
| `contract_diff` | Сравнение двух версий контракта на breaking changes: OpenAPI 3.x (CD-001..CD-006: удалённые пути/операции/обязательные параметры/коды ответов, смена типов полей схем; CD-007 — ломающий дифф без смены major `info.version`), protobuf/gRPC `.proto` (CD-P01..CD-P05 + CD-P06 major пакета `.vN`; удаление, покрытое `reserved`, — допустимо), Avro `.avsc` (CD-A01..CD-A05: поля без `default`, несовместимые типы), JSON Schema топиков (CD-J01..CD-J05: required/properties/типы), DDL-миграции `.sql` (CD-S01..CD-S05: DROP/ALTER TYPE/NOT NULL без DEFAULT). `format=auto` — детектор по расширению/содержимому; `model` — связка с моделью (ADR-035): ломающий дифф сразу возвращает потребителей и владельцев по `contract` у INT. CLI: `arch-be contract-diff <old> <new> [--contract-format] [--format] [--model] [--json]`, breaking → exit 1 | `old`*, `new`* — пути к версиям контракта (yaml/yml/json/proto/avsc/sql); `format` (`auto`/`openapi`/`proto`/`avro`/`jsonschema`/`ddl`); `model` (корень кейса с `model/`) |
...
| `landscape_report` | Ландшафт систем набора проектов (EA-3, ADR-037): агрегация `model/` ROOT и подкаталогов → реестр SYS/INT с дедупликацией по имени, находки (id-divergence, status-conflict, dangling-ref, cross-project-link), топ связности. Толерантная загрузка (E3) — `load_issues`. Ответ — JSON `{format, systems, edges, findings, summary, report}` (мост в MCP, read-only; отчёт, не гейт) | `path`* (корень набора проектов); `format` (`markdown`/`mermaid`, по умолчанию `markdown`) |
| `adr_registry` | Глобальный реестр ADR по набору проектов (ADR-036): `docs/adr/*.md` + `model/ADR-*.md` самого ROOT и подкаталогов; находки — коллизия номеров (между проектами с разными заголовками и **внутри одного проекта**: ≥2 файлов с одним ADR-NNN, нюанс «разные заголовки — конфликт решений под одним номером»/«тот же заголовок — вероятная копия файла», E1), дубль заголовка у разных номеров, пропуск даты/статуса. Ответ — JSON `{passed, entries, findings, summary, report_markdown}` (мост в MCP, read-only); `passed=false` только при `strict=true` и наличии находок (гейт, семантика `--strict` CLI) | `path`* (корень набора проектов); `strict` (по умолчанию `false`) |
| `rule_template_list` | Библиотека шаблонов исполняемых правил (ADR-050): id, версия, что проверяет каждый из девяти шаблонов (восемь паттернов + заготовка). Применение — `rule_template_apply`, проверка зубов — CLI `arch-be rules template verify` | — |
| `rule_template_show` | Шаблон целиком: проверяемые свойства, словарь подбора по тексту инварианта, состав файлов (python/Java), команды прогона и файл нарушающей реализации | `id`* (см. `rule_template_list`) |
| `rule_template_apply` | Положить файлы шаблона в кейс (`skeleton/rule_templates/<id>/`), записать `.arch-handoff/rule-templates.lock` (SHA-256) и вернуть фрагмент правила `CONSTRAINTS.yaml` со свободным `C-NNN` и строкой `verified_by`. Реестр и спайн **не правит** — фрагмент вносит архитектор дельтой; существующие файлы не перезаписывает (ошибка с перечнем конфликтов). Только под `--rw` | `id`*; `ad`* (`AD-3`); `path` (корень кейса); `lang` (`python`\|`java`\|`both`); `dry_run` |
| `rules_report` | Отчёт по реестру правил CONSTRAINTS.yaml: сводка по типам/severity, карточки (owner/expiry/exclude_glob/effort_hours), находки (без owner/expiry, просроченные), git-прокси стоимости. Путь реестра — единый резолвер (E2), в ответе `constraints`, `drift_note` (дрейф двух копий), `skipped_unknown` (неизвестные типы). Ответ — JSON `{rules_total, by_kind, by_severity, summary, report_markdown}` (мост в MCP, read-only; отчёт, `passed` всегда true) | `repo`*; `constraints` (путь к YAML, иначе авто-резолв) |
| `openspec_coverage` | Покрытие требований OpenSpec правилами CONSTRAINTS.yaml (связь — поле `covers:`): SHALL всего / покрыто / unverifiable с owner / без решения, непокрытые поимённо; в ответе также `constraints` (использованный путь) и `drift_note` (пометка дрейфа второй копии реестра, если расходится — E2). Ответ — JSON `{passed, total, covered, unverifiable, unresolved, unresolved_items, summary, report_markdown}` (мост в MCP, read-only); `passed=false` только при `strict=true` и требованиях «без решения» | `path`* (корень репозитория с openspec/); `constraints` (иначе авто-детект); `strict` (по умолчанию `false`) |
| `model_graph` | Граф связей модели (`model/`, ADR-003): text — список сущностей со связями; mermaid — flowchart LR (совместим с `mermaid_render`). Толерантная загрузка (E3) — `load_issues`. Ответ — JSON `{format, entities, edges, summary, graph}` (мост в MCP, read-only; отчёт, не гейт) | `dir` (каталог модели, по умолчанию `model`); `format` (`text`/`mermaid`, по умолчанию `text`) |
