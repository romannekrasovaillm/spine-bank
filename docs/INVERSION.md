...
Статус на 2026-09-19, релиз v0.3.0 (волны 1–3 закрыты: живое тестирование на
пяти харнессах — [HARNESS-TESTS.md](HARNESS-TESTS.md)). План — внешний
документ; здесь — что из него сделано и где смотреть. Все шаги плана,
помеченные обязательными, закрыты; единственный пропуск — опциональный
судья через sampling (см. конец таблицы).
...
| Бэклог волны 3, п. 11: MCP prompts — плейбуки `spine-*` как слэш-команды хоста | ✅ серверная сторона + живые проверки хостов | `src/mcp_server.rs`: capability `prompts`, `prompts/list`/`prompts/get` (9 плейбуков spine-workflows; текст — пользовательская копия из `plugins.dirs`, иначе встроенный ассет); `resources/*` по-прежнему не поддержаны (`-32601`); живые проверки 2026-09-18: Qwen Code 0.24 — 7 плейбуков как команды [Project], Claude Code — prompts/list по NDJSON ([HARNESS-TESTS.md](HARNESS-TESTS.md)) |
...
| 3. `export <host>` → реализовано как **`arch-be connect <host>`** (имя `export` было занято журнальным экспортом) | ✅ | `src/connect.rs`: claude / qwen / gigacode / kimi / omp / codex / generic + хосто-независимые гейты `ci` (gitlab/github/jenkins) и `git-hooks` (pre-commit + pre-push) |
