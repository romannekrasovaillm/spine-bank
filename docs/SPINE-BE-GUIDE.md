...
## 6а. Слой суждения: то, что механика не ловит (0.3.5)

Механика проверяет форму — файл есть, ссылка разрешается, шаблон найден,
число в бюджете. Три класса дефектов она не ловит **по построению**, и до
0.3.5 они были честно названы, но не закрыты: решение противоречит инварианту
при формально верном тексте (`D10`), ссылка ведёт «не на ту» сущность (`D6`),
код нарушает инвариант, а правила на этот код нет (`D11`). Плюс четвёртый,
вскрытый кейсом «Кошелёк ЦР физлица»: показатель обещан, а механизм его не
даёт.

Смысловое ревью — **суждение с доказательствами**, и ядро здесь остаётся
детерминированным: Spine собирает досье и сверяет цитаты, а судит модель
...
- **Обвинение и похвала доказываются по-разному.** Низкий балл смысловой
  рубрики — это найденное противоречие, и он обязан опираться на цитаты
  ОБЕИХ сторон. Высокий балл («противоречий нет») цитатой не доказуем —
  отсутствие не цитируется, поэтому судья перечисляет, что именно проверил,
  а механика сверяет перечень с составом досье.
- **Выдуманное обвинение наказывает судью, а не документ.**
  `accusation_unconfirmed` — критерий исключается из итога, гейт не краснеет.
  Ложное обвинение хуже пропуска: оно учит не писать исключений вовсе.
- **Отчёт привязан ко всем источникам досье.** Правка спайна — не субъекта! —
  обесценивает отчёт о согласованности: `semantic_report_stale` называет
  изменившийся источник поимённо.
- **Смысловая строка red-team не входит в долю обнаружения.** «11 из 14» и
  порог 78 % — про механику; «поймано k из 3» — про суждение. Это разные
  вопросы, и смешивать их значило бы мерить одно другим.
- **Составляющая `semantic_quality` выключена по умолчанию** (ADR-052):
  её включают строкой в `[gate.required]`, потому что оценка дорогая (k
  сэмплов судьи на субъект), а ужесточение без предупреждения ломает чужие
  пайплайны. Паспорт вердикта печатает её состояние и подсказывает, какая
  рубрика закрывает какой класс.

Методика — скилл `semantic-review`; порядок прогона — плейбук
`spine-semantic-judge`; решения — ADR-051 и ADR-052.
