...
2. **`handoff_create`** собирает пакет `<repo>/.arch-handoff/`:
   ...
   - `CONSTRAINTS.yaml` — **копия корневого** реестра проекта, если он есть
     (иначе заготовка под стек репозитория); генератор валидирует текст как
     YAML до записи — битый файл исполнителю не уезжает. Копия, а не
     заготовка, потому что пакетная копия приоритетна для резолвера гейта:
     заготовка в пакете молча переключала гейт на одно правило вместо
     реестра проекта. Существующий файл пакета не перезаписывается — правки
     архитектора сохраняются; перезапись — явным `--refresh-constraints`.
     Разошедшиеся копии гейт называет находкой `registry_diverged`;
