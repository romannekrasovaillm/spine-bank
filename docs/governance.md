...
Порядок цикла: предложить (`new`) → влить правки → **merge в основную ветку** →
`archive`. Архивация на ветке до merge тоже проходит: дельта покрывает правки
своего диапазона `base..HEAD` (T-07), — но правки, попавшие в базу ДО архивации,
остаются без активной дельты, и гейт от базы справедливо краснеет. На невлитой
ветке `arch-be delta archive` печатает подсказку об этом порядке.
