...
> Что получится в итоге: агент GigaCode сможет вызывать 33 детерминированных
> инструмента Spine (fitness-гейты, трасса, модель, контракты, NFR, реестры,
> рубрики), видеть 62 архитектурных скилла и останавливаться на красном гейте.
...
<<<<<<< HEAD
   MCP-сервер `spine` в `<каталог настроек>/settings.json` + 64 скилла в
=======
   MCP-сервер `spine` в `<каталог настроек>/settings.json` + 65 скиллов в
>>>>>>> origin/main
...
1. Бинарь:
   ...
   - ВАРИАНТ А (быстрый, готовый бинарь из GitHub Releases):
     ...
     (создай ~/.local/bin при нужде; сверь sha256 с файлом SHA256SUMS из релиза).
   ...
   - Проверь: `arch-be --version` (ожидается 0.3.x).
...
<<<<<<< HEAD
     И разложит 64 скилла в `<каталог настроек>/skills/` (каталог выбирается
=======
     И разложит 65 скиллов в `<каталог настроек>/skills/` (каталог выбирается
>>>>>>> origin/main
