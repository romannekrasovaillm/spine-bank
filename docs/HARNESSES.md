...
## Происхождение оценки: чей судья

Рубрику в чужом харнессе можно получить двумя способами, и отчёт об этом
говорит прямо (ADR-048):

- **судью запускает Spine** (`kind = "cli"`, способ A) — в отчёте
  «независимость **обеспечена запуском**»: известны команда, аргументы и то,
  что каждый сэмпл — отдельный подпроцесс;
- **судит хост** (split-judge, способ B) — «**заявлена**»: метки передал хост,
  механика их не проверяет, зато видно имя и версию хоста, число вызовов в
  сессии до судейства и хэш выданного промпта.

Паспорт вердикта (`arch-be gate --explain`, блок 2) печатает это отдельными
строками, а гейт умеет требовать уровень: `[gate.decision_quality]
min_independence = "launched"`. Передача судейства и приёмка результата —
`arch-be rubric handover` / `arch-be rubric accept` (плейбук
`spine-judge-handover`), подключение судейского харнесса — `connect <хост>
--rw=reports`.
