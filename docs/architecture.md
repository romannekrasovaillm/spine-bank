...
| `rubric/` (`judge`,`report`,`types`,`decision`,`artifact`,`human_decision`,`qualify`,`portfolio`,`catalog`,`tools`) | Движок смысловых рубрик (ADR-004, ADR-051): якорные/динамические рубрики, LLM-судья (k сэмплов → медиана, метки `unstable`/`evidence_not_found`/`accusation_unconfirmed`/`invalid_samples`), механическая проверка цитат и цитат с указателем на источник (E9.1: роль берётся из состава досье — `citation_role_mismatch`, `citation_source_unknown`), поимённые вердикты по сценариям `OpenSpec` (E11.1), сканер prompt-инъекций входа (E2), единое решение `pass`/`fail`/`human` и его коды выхода (E4), пакет архитектору и решение человека, привязанное к ревизии отчёта (E4.5), второй судья другой модели (E5.1), квалификация судьи на эталонном наборе (E6), смысловой срез портфеля и пакет комитета (E10.3/E10.4), адаптивные сэмплы (E8.1), `RubricReport` и артефакт с происхождением и сырыми ответами. |
| `judge_cache.rs` | Кэш вердикта судьи (E8.2): ключ — хэш рубрики, модели, досье/документа и настроек; попадание видно в `provenance.cache`, обход — `rubric run --no-cache`, кэш ограничен и переживает испорченный файл. |
| `judge_rules.rs` | История отчётов судьи → кандидаты в детерминированные правила (E7.3): повторяющееся обвинение даёт YAML `must_not_contain`, повтор метки механики — advisory; JSON-близнец отчёта пишется рядом с markdown. |
| `openspec.rs` | Адаптер `OpenSpec`: требования (`### Requirement:` + SHALL/MUST, стабильные идентификаторы `openspec:<capability>#<hash8>`) и сценарии проверки (`#### Scenario:` → WHEN/THEN, `…/S<n>`) — основа досье `code_vs_scenarios` (E11.2) и покрытия правил. |
| `metrics.rs` | Операционные метрики (`arch-be metrics`): сессии, инструменты, токены, рубрики; смысловой срез стоимости ревью (E8.3) — время судьи и токены на решение из JSON-близнецов отчётов. |
