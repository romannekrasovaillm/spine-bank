...
|---|---|---|---|
| `OpenAPI` 3.x | yaml/json | CD-001 удалённый путь, CD-002 операция, CD-003 обязательный параметр удалён/стал required, CD-004 код ответа, CD-006 смена типа поля схемы, CD-008 поле стало обязательным в `requestBody` (`$ref`, `allOf`, вложенные объекты — T-06), **CD-007 ломающий дифф без смены major `info.version`** | CD-005 добавленные пути/операции/необязательные параметры/коды |
