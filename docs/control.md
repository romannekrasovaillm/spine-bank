...
`git diff` по базе: голая ревизия получает `...HEAD`, готовый диапазон
используется как есть (T-03: раньше `origin/main...HEAD` дополнялся вторым
`...HEAD`, git отказывал, и гейт молча уходил в fail-safe Critical). Вне
git-репозитория — ошибка с понятным текстом.
...
## Мутационное тестирование пакета (`redteam`)

Отдельная команда, не составляющая гейта: `arch-be redteam <кейс>` засеивает по
одному дефекту из каталога раздела 7 ТЗ 0.3.4 и печатает карту обнаружения.
Считает **защищённость пакета правилами**, а не вердикт по репозиторию; exit 1,
если доля обнаружения ниже порога. Подробности — `docs/redteam.md`.
...
раздел. Модель доверия (A3, ADR-053) добавляет `untrusted_skipped[]`
(`{rule, severity, reason}`, reason с префиксом `command_untrusted`) — см.
раздел ниже.

## Модель доверия `command_succeeds` (A3, ADR-053)

Правило `command_succeeds` исполняет `bash -c <command>` из `CONSTRAINTS.yaml`
проверяемого репозитория. Для своего репозитория это нормально; для чужого —
нет (гейт на PR из форка, MCP-сервер у агента на незнакомом клоне, `control
check` по кейсу коллеги). Исполнение начинается с решения о доверии:

- **no-exec**: флаг `--no-exec` у `control check` / `gate` / `review` либо
  переменная окружения `ARCH_NO_EXEC=1` (явное `0` — выключить; так же ведут
  себя `true/yes/on`/`false/no/off`). Запрещённые правила не запускаются
  вообще: в отчёте — пропуск `command_untrusted` с подсказкой, как разрешить.
  **MCP-сервер (`arch-be mcp serve`) работает с no-exec=вкл по умолчанию**;
  отключить — явным `ARCH_NO_EXEC=0` в окружении сервера.
- **allow-файл (opt-in, TOFU)**: `arch-be rules allow [<repo>]` записывает в
  `~/.arch-harness/trusted.json` (права 0600) отпечаток SHA-256
  канонизированного набора command-строк всех `command_succeeds`-правил
  разрешённого реестра (канонизация: строки вербатим, байтовая сортировка,
  склейка `\n`). Пока записи нет — поведение прежнее (исполнение). Запись не
  совпадает — реестр менялся после доверия: пропуск `command_untrusted`,
  пока доверие не подтвердят повторно (`rules allow`).
- no-exec приоритетнее allow-файла: сессионный запрет сильнее постоянного
  разрешения.

Семантика пропуска: не находка и не провал — `passed` и exit-код `control
check` не меняются (пропуски видны в сводке и в поле `untrusted_skipped`).
В гейте пропуск по доверию ЛЮБОГО `command_succeeds`-правила переводит
составляющую `fitness` в SKIP с деталью `command_untrusted` и именами правил
(строже A2-пропусков по отсутствию прогонщика, где блокируют только
error-правила: пропуск по решению политики — не разрыв окружения, и зелёный
PASS при неисполненных командах был бы молчаливой ложью); обязательная
составляющая в SKIP даёт INCOMPLETE (exit 3), а блок 3 паспорта вердикта
(`arch-be gate --explain`, MCP `verdict_explain`) перечисляет пропущенные
правила с причиной. Аудит путей к исполнению —
...
Правило на упоминание (`must_contain` по документам) зеленеет и когда инвариант
соблюдён, и когда о нём просто написали. Шаблон сокращает путь до проверки
поведения: готовое правило `command_succeeds`, тест свойства на фейке внешней
системы, эталонная реализация (тест зелёный из коробки) и **нарушающая** — на
ней тест обязан падать.

```bash
arch-be rules template list                      # библиотека: 9 шаблонов
arch-be rules template show idempotency-key      # свойства, файлы, команды, словарь подбора
arch-be rules template apply idempotency-key --ad AD-001 --dir . --lang python
#   → файлы в skeleton/rule_templates/idempotency-key/, lock с SHA-256,
#     напечатан фрагмент правила (id C-NNN, ad: AD-001) и строка verified_by
arch-be rules template verify --all --require-python   # зубы всей библиотеки
arch-be rules template verify --dir .                  # зубы применённых в кейсе
```
...
входит; печатается командой и разделом в `control rules-report`). Если файлы
кейса разошлись с lock-файлом или команда правила переключена с шаблона на
настоящий код, проверка честно говорит «адаптирован: проверьте вручную» вместо
ложного «проверено».

**Стоимость.** Применённое правило исполняется в **каждом** прогоне гейта,
включая Stop-хук и pre-push: `timeout_secs` шаблона — 120, типичный набор
свойств — меньше 0.1 с.

Java-половина шаблонов прогоняется Maven'ом из `PATH`, а без него — JUnit-консолью
флагом `--java-jar <junit-platform-console-standalone.jar>` (бинарный jar в git не
кладём: ассеты встраиваются `include_str!`). Нет ни того, ни другого — половина
...
**Заготовка не удостоверяет откат** (Д8, ADR-049). `arch-be bootstrap` кладёт
в пакет `REHEARSAL.json` с `"passed": false` — это образец формы, а не
результат прогона. Семантика бандла (`arch-be evidence verify`) ловит два
случая, на которых аттестация расходится с фактом:

- `rehearsal_empty` — отчёт объявлен пройденным, но не содержит ни одного
  шага (severity маршрута: на Critical error);
- `rehearsal_baseline_unresolved` — якорь отчёта не резолвится в коммит
  этого репозитория (всегда `warn`: пакет мог быть собран в другой истории).

Проверка якоря смотрит только «пройденные» отчёты: настоящая репетиция
непройденного отчёта с неразрешимым якорем произвести не может — `rehearse`
падает на этом якоре до записи evidence, — поэтому `passed: true` с таким
якорем всегда означает правку отчёта руками.
...
в теле хотя бы одной дельты-покрытия: АКТИВНОЙ `changes/*/DELTA.md` либо
**заархивированной ВНУТРИ проверяемого диапазона** `base..HEAD` (T-07). Дельта,
заархивированная ДО базы, покрытием не становится: влитая истина не освобождает
от протокола для новых правок, а архив, засчитанный вне своего диапазона, был бы
универсальной отмычкой.

Порядок «merge → archive» предпочтителен: архивация на feature-ветке до merge
делает правки диапазона покрытыми, но правки, попавшие в базу ДО архивации, —
уже нет. На ветке, не влитой в основную, `arch-be delta archive` печатает
подсказку об этом порядке.
...
## Проводник: `arch-be bootstrap`

```bash
arch-be bootstrap "Имя кейса" [--dir каталог] [--domain payments]   # каркас + первый шаг
arch-be bootstrap --status [--dir каталог]                          # прогресс и следующий шаг
```

Создаёт каркас кейса и ведёт к зелёному: четыре стадии
(`спайн → правила → модель → бандл`), после каждой — следующая красная
находка с `fix_hint`. Каркас содержит спайн с двумя примерами инвариантов,
три правила, минимальную связную модель (SYS/CAP/REQ/NFR/AD/ADR/CMP/INT/OWNER),
`ROUTE.lock`, первый ADR по шаблону, план отката и отчёт репетиции.

Границы проводника:

- **Каркас красный by design.** Артефакты бандла — заглушки, и их обязана
  ловить семантика Н1 (`evidence_stub`). Иначе проводник производил бы
  ложнозелёные пакеты — «выпуск разрешён» за «ничего не написано». Тест
  `bootstrap_stub_skeleton_is_caught_by_evidence_semantics` держит границу.
- **Занятый каталог — отказ**, а не дозапись: для существующего кейса есть
  `--status`.
- **Git.** Вне репозитория каркас становится git-репозиторием с одним
  коммитом — дельта-протокол и `delta_guard` (обязателен на Critical) работают
  от базы. Внутри чужого репозитория коммит делает вызывающий, и проводник об
  этом говорит.
- **Ничего за человека.** A3 не подписывается, решение не пишется, составляющие
  гейта не включаются.

## Метрика доверия: `arch-be trust`

```bash
arch-be trust [каталог] [--format json]
```

Положение контура на шкале **1–5 с якорями и доказательствами**, а не число.
Число «4 из 5» бесполезно: непонятно, что проверено и чего не хватает. Поэтому
у каждой ступени два обязательных поля — чем подтверждена (файл, счётчик,
вердикт) и почему не достигнута.

| Якорь | Чем подтверждается |
|---|---|
| 1. Контур подключён | `.arch-handoff/mcp-calls.jsonl`: вызовы и число разных инструментов |
| 2. Гейт останавливал работу | в журнале есть `fail`, после которого тот же инструмент дал `pass` |
| 3. Правила сопровождаются | у всех правил владелец и срок, нет просроченных, ≥1 правило проверяет поведение; пометки FP — рядом |
| 4. Пакет защищён измеренно | `.arch-handoff/redteam.json` (пишет `arch-be redteam <кейс> --save`): доля обнаружения ≥ порога и контроль аттестации |
| 5. Вердикт полон и подписан | нет обязательных составляющих без входа, нет неподписанного A3, судья рубрики ≠ автор |

Честные правила шкалы:

- **Якорь без доказательства не засчитывается.** «Правила сопровождаются» без
  прочитанного реестра — обещание, а не факт.
- **Ступень 5 недостижима**, пока есть блокеры: неподписанное A3, судья-автор,
  доля обнаружения ниже порога (или не измерена). Блокеры названы поимённо с
  адресом.
- **Метрика ничего не блокирует**: она отвечает на вопрос «насколько можно
  верить зелёному этого контура», а не «можно ли выпускать». Она дорога ровно
  настолько, насколько дорог прогон гейта (он нужен для якоря 5).
- Источники — журнал MCP-вызовов, реестр правил и регистр FP, сохранённый
  результат `redteam`, вердикт гейта, отчёты рубрик. Два разных мнения об
  одном состоянии не поддерживаются: блокеры и якорь 5 читают один источник.
...
|---|---|---|---|
| `OpenAPI` 3.x | yaml/json | CD-001 удалённый путь, CD-002 операция, CD-003 обязательный параметр удалён/стал required (в том числе за `$ref` — Д5), CD-004 код ответа, CD-006 смена типа поля схемы, CD-008 поле стало обязательным в `requestBody` (`$ref`, `allOf`, вложенные объекты — T-06), CD-009 поле удалено из тела ответа (поддерево называется минимальным путём — Д5), CD-010 тело запроса стало обязательным (`requestBody.required: false → true` — Д5), **CD-007 ломающий дифф без смены major `info.version`** | CD-005 добавленные пути/операции/необязательные параметры/коды |
...
**Направление у тел разное** (Д5, ADR-046). Тело запроса ломает то, что
заставляет потребителя прислать больше (`CD-008` — новое обязательное поле,
`CD-010` — тело стало обязательным). Тело ответа — наоборот: потребитель его
читает, поэтому ломает исчезновение поля (`CD-009`), а появление нового
обязательного поля ответа безопасно и находки не даёт. Удаление поля уносит
поддерево: сообщается минимальный путь (`wallet`), а не каждый потомок
(`wallet.id`, `wallet.label`) — иначе рекурсивные схемы дают десяток находок
об одном удалении. Параметры за `#/components/parameters/…` резолвятся тем же
`$ref`-резолвером, что схемы тел.
