...
отдаёт десять плейбуков `spine-*` (`spine-architect-review`, `spine-fitness-gate`
...
```bash
arch-be mcp serve          # дефолт: строго read-only (40 инструментов:
                           #   16 ручных + 24 моста в реестр)
...
### Что попадает в отчёт: происхождение оценки (0.3.5, ADR-048)

Отчёт рубрики несёт **проверяемое происхождение**, и паспорт вердикта
(`arch-be gate --explain`) различает два уровня независимости:

| Способ | Уровень в отчёте | Что механика знает |
|---|---|---|
| A. `kind = "cli"` | **обеспечена запуском** | Spine сам запускал судью: команда, аргументы, каждый сэмпл — отдельный процесс. Какая модель отвечала — не знает |
| B. split-judge | **заявлена** | метки передал хост (видно имя и версию хоста), число вызовов в сессии до судейства, хэш выданного промпта |

Ни один уровень не означает «независимость подтверждена»: механика называет
то, что знает, и не додумывает остальное. Дополнительно отчёт несёт хэши сырых
ответов судьи, поэтому балл, поправленный в отчёте руками, ловится гейтом
(`rubric_report_inconsistent`), а подмена сохранённого ответа —
`rubric_raw_tampered`.

**Передача и приёмка судейства** (когда решения кейса не оценены):

```bash
arch-be rubric handover --dir <кейс>   # что осталось оценить и чем + два текста передачи
arch-be rubric run adr_quality --all-accepted --model <судья>   # способ A сразу по кейсу
arch-be rubric accept --dir <кейс>     # приёмка: свежесть, судья, уровень, сходимость ответов
```

Плейбук `spine-judge-handover` ведёт по тому же пути изнутри агента
(`rubric_handover` → выбор способа → `rubric_accept` → гейт).

> **Подключайте судейский харнесс с узкой записью:** `arch-be connect claude
> --rw=reports`. В read-only отчёт не сохранится (гейт скажет
> `rubric_report_missing`), а широкий `--rw` открывает ещё девять пишущих
> инструментов, которые судье не нужны.
...
| Гейт: `rubric_report_missing`, хотя оценка прошла | Отчёт не сохранён: хост подключён read-only. Переподключите с `--rw=reports` или сохраните `artifact_json` из ответа `rubric_verify` по `artifact_path` |
