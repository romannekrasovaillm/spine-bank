...
fn prompts_list_and_get_over_stdio() {
    ...
    // prompts/list: ровно десять плейбуков spine-workflows (дом изолирован —
    // тексты и описания из встроенных ассетов).
    let prompts = responses[1]["result"]["prompts"]
        .as_array()
        .expect("prompts");
    assert_eq!(prompts.len(), 10, "десять плейбуков: {prompts:?}");
...
/// J5 (ADR-048): порог независимости. По умолчанию (`none`) поведение 0.3.4
...
/// J5 (ADR-048): судейство в рабочей сессии — ПРИМЕЧАНИЕ паспорта, а не
...
/// J5 (ADR-048): хэши сырых ответов входят в аттестацию вердикта — правка
