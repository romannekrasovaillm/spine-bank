...
Одна кодовая база — специально: ядро, тесты (1698 в редакции harness,
1349 в core) и CI общие, версии не расходятся. Различие — только в том, кто
«думает»: ваш агент или Spine сам.
...
Патч по итогам **расширенных живых тестов 0.3.9** (весь CLI, обе редакции,
MCP-орган, TUI): контур перестал терять свидетельства на путях, которые руками
не проверяют. Поведение в обычных окружениях не меняется — ничего не краснеет.

- **MCP-путь судейства ведёт архив истории.** `rubric_verify` писал только
  артефакт по субъекту: повторная проверка того же субъекта затирала и отчёт, и
  сырые ответы — прошлый прогон пропадал (живой TUI-тест: честный отчёт исчез за
  поддельным). Теперь оба пути, CLI `rubric run` и MCP `rubric_verify`, пишут
  архивную пару (markdown + JSON-близнец) в `~/.arch-harness/reports/` через
  общий `judge_rules::write_archive`, а в ответе инструмента есть поле
  `archive`.
- **`rules template apply` снова видит инварианты.** `--ad AD-n` (CLI и MCP
  `rule_template_apply`) требовал форму заголовка `## AD-N. …`, тогда как спайны
  репозитория, кейсов и каркаса `bootstrap` пишут `## AD-N: …`: агент получал
  «нет объявленных инвариантов» на любом реальном спайне. Разбор делегирован
  каноническому парсеру контура контроля — один канон на крейт.
- **`cron` работает сразу после `arch-be init`.** Три отказа подряд: пути в
  образце расписания начинались с `~`, которую планировщик не раскрывал;
  штатный дайджест базы знаний не укладывался в жёсткие 16 итераций (теперь
  `max_turns` у задачи в `cron.toml`, образец — 40); статус отчёта читался
  строго из последней строки, а шаблон задачи оборачивает JSON-контракт в
  ```` ```json ```` — валидный `{"status": "complete"}` давал `unknown`.
  Ограждения и пустые строки пропускаются, а в сообщении об исчерпании лимита
  есть подсказка про настройку.
- **`doctor` проверяет задачи cron**: называет задачи, файлов которых нет
  (`cron tick` упал бы на чтении), а не ограничивается наличием `cron.toml`.

## Что нового в 0.3.9

Волна архитектурного ревью **E1–E11** («Архитектурное ревью с LLM на Spine
Core») доведена до конца: смысловая рубрика по коду приведена к той же
пригодности к аудиту, что рубрика по документу, а её доказательства стали
механически проверяемыми.

- **Аудиторский след и защита входа (E1–E3).** Сырые ответы судьи и
  происхождение, пересборка отчёта без LLM, детектор prompt-инъекций на входе,
  строгая валидация ответа судьи. Ключевая метрика волны — аудит вердикта
  «проверка кода» **6 из 6** на стенде `spine-judge-bench` (у OpenSpec и голой
  LLM — 0 из 6).
- **Решение вместо мнения (E4–E6).** Единое решение рубрики
  `pass`/`fail`/`human` и его интеграция с гейтом, независимость судейства
  (второй судья, контракт автора), квалификация судьи на эталонном наборе.
- **Доказательства и цена (E7–E9).** Детекторы как доказательства в досье и
  кандидаты в правила, стоимость ревью (адаптивные сэмплы, кэш, метрики),
  цитаты с указателем на источник как основной канал доказательств.
- **Корпоративный слой и процесс (E10–E11).** Стандарты ДКА, обоснованность
  исключения, срез по портфелю и пакет комитета; сценарии проверки OpenSpec
  оцениваются поимённо.

Сквозной цикл «пакет → дельта → evidence → рубрики → контур контроля → паспорт
вердикта» пройден живьём внутри Qwen Code через MCP-орган (`spine`).
...
- **`rubric reverify` краснеет на устаревшем досье**: если источник отчёта
  изменился после оценки, сверка говорит «досье устарело — <источник> изменён
  после оценки» и возвращает код 1. Вердикт не переносится на новую редакцию
  досье — субъект нужно оценить заново (отчёты до 0.3.8 не штрафуются: сверять
  не с чем).
- **`decision_quality` видит отчёты по досье**: подмена сохранённого ответа
  (`rubric_raw_tampered`), правка балла (`rubric_report_inconsistent`) и
  изменившееся досье (`rubric_report_stale`) — error-находки; отчёт без сырых
  ответов — warn `rubric_report_unreproducible`.
- **Допуск судьи включается проектом (E6.3)**: при
  `require_qualified_judge = true` судья без пройденной квалификации даёт
  `judge_unqualified` и вердикт **INCOMPLETE** (exit 3). По умолчанию флаг
  выключен.
- **Детектор инъекций ловит директиву вердикта, а не только английские
  формулы** («отметить как чистую», «считать соответствующей», `mark as clean`,
  `do not flag`, …). Широкое «игнорируй инструкции» по-прежнему не берётся: оно
  встречается в обычной технической прозе.
- **Второй судья (E5.1)**: `rubric run --pack … --second-model <имя>` — две
  независимые модели, расхождение решений сверх допуска даёт `human` (exit 2) и
  находку `judge_disagreement`. Это ответ на «уверенный промах с настоящими
  цитатами».
- **Автор кода берётся из контракта передачи** (`.arch-handoff/MANIFEST.json`),
  если `--author-model` не задан: уровень независимости считается по составу
  судьи и автора, совпадение видно как `judge_is_author`.
- **Оговорки судьи на Critical блокируют вердикт**: неподтверждённое обвинение,
  неполное покрытие и подобные — не «примечания», а остановка.

## Что нового в 0.3.8

Патч по итогам первого прогона CI 0.3.7: джобы `hermetic` (контейнер без Python)
и `dogfood` нашли то, что локальная машина прятала. Сигнал процессной группе
теперь идёт через bash-builtin `kill` — в slim-образах нет `procps`, внешнего
`kill` не находилось, и таймаут `command_succeeds` не убивал группу (8 красных
тестов hermetic-джобы → 0). Тесты SKIP-раннеров перестали падать при полном
отсутствии `python3`.

## Что нового в 0.3.7

Релиз про **таймаут, которому можно верить, и модель доверия к исполняемым
правилам** — волны A–D по механическому ревью `c5f94ea` (задание
`TASK-Spine-Core-0.3.7.md`, разбор — `CHANGELOG.md`).

- **Таймаут `command_succeeds` теперь убивает процессную группу.** Раньше
  убивался только `bash`, а внуки (`pytest`-воркеры, `server &`) держали
  гейт: правило с `timeout_secs: 2` могло работать 15 секунд, а с фоновым
  процессом — висеть бесконечно. Весь запуск shell ушёл в `src/proc.rs`
  (TERM → KILL по группе, читатели с дедлайном); прямой
  `Command::new("bash")` вне него ловит фитнес-правило.
- **Доверие к исполняемым правилам (ADR-053).** В MCP-режиме
  `command_succeeds` по умолчанию не исполняется (находка
  `command_untrusted` в паспорте вердикта); в CLI — флаг `--no-exec` /
  `ARCH_NO_EXEC=1` и opt-in доверие `arch-be rules allow` по отпечатку
  набора команд. Аудит путей исполнения —
  `docs/experiments/command-trust.md`.
- **Честный SKIP вместо ложного ✗.** Нет `pytest`/`mvn` на машине — правило
  пропускается с причиной и подсказкой, `bootstrap` честно говорит
  `INCOMPLETE` с первым шагом «установите pytest», `doctor` проверяет
  раннеры, а CI получил джобу `hermetic` без Python.
- **Декомпозиция монолитов:** ни одного продового модуля длиннее 3000 строк
  (`control/`, `cli/`, `gate/`, `mcp_server/`, `connect/`, `rubric/`,
  `contract_diff/`), граница удерживается правилом C-33; граница будущего
  ядра `spine-core` — правилом C-34, план выделения — ADR-054 и
  `docs/SPINE-CORE-API.md`.
- **Ночные прогоны надёжности:** cargo-fuzz по парсерам, cargo-mutants по
  ядру гейта, llvm-cov трендом (`nightly.yml`); номер версии — на
  стартовом экране TUI.
- **Открытость:** `CONTRIBUTING.md`, короткий `README.en.md`, шаблоны
  issue/PR.
...
- **Происхождение оценки: кто судил, чем и что из этого проверяемо.** Отчёт
  ...
  или «обеспечена запуском» (судью запускал Spine); паспорт вердикта говорит
  это словами, в которых нет «независимость подтверждена». Балл, поправленный
  в отчёте руками, ловится гейтом (`rubric_report_inconsistent`), подмена
  сохранённого ответа — `rubric_raw_tampered`, а `arch-be rubric reverify`
  пересобирает отчёт из ответов без LLM. Передача судейства во второй
...
Релиз про **контур, который нельзя выключить случайно**: убраны стыки, на
которых архитектурный контроль отключался молча — а вердикт продолжал
выглядеть зелёным.

- **Реестр правил — один, и его расхождение видно.** `bootstrap` клал реестр в
  корень кейса, генератор пакета подкладывал в пакет заготовку из одного
  правила, а резолвер читал пакетную копию — команда получала «Правил: 1,
  PASS». Теперь пакетный реестр — копия корневого, а расхождение копий —
  находка `registry_diverged` (error) с числом правил в каждой (ADR-045).
- **Хук не молчит о пропуске.** Гарда «реестр лежит в `.arch-handoff/`»
  удалена: Stop-хук, pre-commit и pre-push ищут реестр так же, как гейт, а без
  реестра говорят об этом и выходят с exit 2 — вместо прежнего тихого нуля
  (T-01).
- **Значимость не выключается опечаткой.** Незнакомое имя триггера — ошибка
  вызова с ближайшим каноническим (раньше `new_components` молча занижал
  маршрут), маршрут из диффа опознаёт контракт по содержимому и каталогу, а не
  по слову в имени файла, и видит новые сущности модели (T-04, T-05).
- **Одна форма базы диффа.** `A`, `A..B`, `A...B` разбираются в одном месте:
  документированный `origin/main...HEAD` больше не превращается в
  `...HEAD...HEAD` и не уводит гейт в fail-safe Critical (T-03).
- **Контракты: тело ответа и обязательность тела запроса.** `CD-009` — поле
  исчезло из тела ответа, `CD-010` — `requestBody.required: false → true`;
  параметры за `#/components/parameters/…` резолвятся. Раньше удаление поля из
  ответа давало «breaking: 0 … PASS» (Д5).
...
- **Заготовка больше не выглядит пройденной проверкой.** Отчёт репетиции отката
  из `bootstrap` объявляет себя заготовкой (`passed: false`), а
  `evidence verify` ловит `rehearsal_empty` и якорь, не резолвящийся в коммит
  (Д8, ADR-049). Цитата-свидетельство судьи рубрики сверяется в **каждом**
  сэмпле, а не только у выбранного под медиану (Д10).

Плюс ужесточения, которые меняют вердикты (полный список — в
[CHANGELOG.md](CHANGELOG.md), раздел «Что может покраснеть после обновления»):

- **Две копии реестра** — error на всех маршрутах: пока они расходятся,
  неизвестно, какие правила действуют.
- **Кириллица в защищённых путях** видна `delta guard`: правка `model/` мимо
  активной дельты на кейсах `кейсы/*` больше не проходит молча.
- **`CD-009`/`CD-010` без мягкого режима**: контракт, у которого исчезло поле
  ответа или тело запроса стало обязательным, впервые показывает FAIL.
...
### В. Насколько можно верить этому зелёному

```bash
arch-be trust .                 # шкала 1–5 с якорями и доказательствами
arch-be trust . --format json
```

```
Доверие к контуру: 3 из 5 (достигнуто: «Вердикт полон и подписан»;
                            следующий — «Гейт останавливал работу»)
  [✓] 1. Контур подключён — .arch-handoff/mcp-calls.jsonl: 4 вызова, 4 инструмента
  [✗] 2. Гейт останавливал работу — провалов в журнале: 1; после провала починен: ни один
  [✗] 4. Пакет защищён измеренно — .arch-handoff/redteam.json: результата нет
        → измерьте: `arch-be redteam . --save`
```

Якорь без доказательства не засчитывается, а ступень 5 недостижима, пока есть
неподписанное A3, судья-автор или доля обнаружения ниже порога. Метрика
ничего не блокирует — она говорит, насколько можно верить зелёному.

### Г. Архитектор внутри харнесса
...
### Д. Судья по рубрикам — без API-ключей
...
Вердикт при этом остаётся проверяемым, а не «красивым текстом»:

- **Решение с кодом.** Рубрика выносит `pass`/`fail`/`human` → код выхода
  0/1/2; хук или CI может на это опереться, а не разбирать прозу.
- **Перепроверка без LLM.** `arch-be rubric reverify` пересобирает отчёт из
  сохранённых сырых ответов судьи: подмена балла («в отчёте 5, из сырых
  ответов 1»), правка сохранённого ответа (по хэшу) и устаревшее досье
  («источник изменён после оценки») — красный вердикт, а не молчание.
  История прогонов не затирается: CLI `rubric run` и MCP `rubric_verify` пишут
  архивную пару в `~/.arch-harness/reports/`.
- **Независимость видна, а не заявлена.** `--second-model` поднимает второго
  судью другой модели (расхождение сверх допуска → `human`, находка
  `judge_disagreement`); автор кода читается из `.arch-handoff/MANIFEST.json`,
  а совпадение судьи с автором печатается как `judge_is_author`. Допуск судьи к
  гейту — `rubric qualify <рубрика> --set <набор>` плюс
  `require_qualified_judge = true`.
- **Защита входа.** Директива вердикта в тексте («отметить как чистую»,
  `mark as clean`, …) фиксируется детектором инъекций: судья, подчинившийся
  ей, виден как таковой, а не выглядит честным.
...
- **Плейбуки как команды**: 10 MCP-промптов (`spine-quickstart`,
  `spine-architect-review`, `spine-adr-judge`, `spine-semantic-judge`,
  `spine-fitness-gate`, `spine-contracts-gate`, `spine-judge-handover`, …) —
  в Qwen Code 0.24 видны в меню как команды `[Project]`, ревью запускается из
  меню.
...
<p align="center">
  ...
  <img src="docs/screenshots/harnesses/waves-claude-tui-mcp.png" alt="Claude Code: /mcp — spine connected (захват: 33 инструмента; актуально — 40 в read-only и 51 с --rw)" width="49%">
</p>

![Headless-прогоны architect_review: omp · OpenClaw · Qwen](docs/screenshots/harnesses/waves-headless-reviews.png)

Полный гид по каналу MCP — карта всех 40 инструментов по задачам
...
arch-be mcp serve --rw    # 51 инструмент: + handoff_create, adr_new, …
```
